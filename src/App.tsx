...
const ObjModel = memo(
  ({ setModelLoaded, model, setModelOrientation }: ObjModelProps) => {
    const { scene, camera } = useThree()
    const [selectedVertex, setSelectedVertex] = useState<Three.Vector3 | null>(
      null
    )
...
function App() {
  ...
  const [modelOrientation, setModelOrientation] = useState(
    new Three.Quaternion()
  )
  const [progress] = useState(0)
  const containerRef = useRef<HTMLDivElement>(null)
  const initialModel = useMemo(
    () => ({ mtlFile: '/uploads/Assem1.mtl', objFile: '/uploads/Assem1.obj' }),
    []
  )
